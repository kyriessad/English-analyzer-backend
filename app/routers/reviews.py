...
def _create_or_return_session(
    ...
    *,
    ...
    # Phase 8L: batch size is independent from today's remaining goal.
    goal_mode = False
    ...
    if session_type == "daily_suggested" and daily_goal is not None:
        ...
        session_limit = min(session_limit, target)
        ...
        completed_unique_today = len(today_reviewed_card_ids)

        remaining = max(target - completed_unique_today, 0)
        goal_mode = remaining > 0
    ...
    selected_cards = _select_session_cards(
        ...
        today_reviewed_card_ids=today_reviewed_card_ids,
        goal_mode=goal_mode,
        fill_target=session_limit if session_type == "daily_suggested" and daily_goal is not None else None,
    )
