...
def _call_and_validate(
    ...
    retry_eligible is True when a stricter prompt might fix the issue
    (i.e. sentence was valid but didn't contain the original word).
    """
...
def generate_example_with_hunyuan(
    ...
    First attempt allows inflections. If the sentence does not contain the
    original word, retries once with a stricter prompt demanding the exact word.
    ...
    try:
        ...
        # First attempt — normal prompt, allows inflection
        ...
        # Only retry when the model produced a valid sentence but used a synonym/inflection
        if not retry_eligible:
            return None, None

        logger.info("[hunyuan] retrying with strict prompt for '%s'", text)
